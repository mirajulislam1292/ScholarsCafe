          ...
          <div className="col-span-2">
            <Link to="/" className="flex items-center gap-2">
              ...
              <span className="footer-brand font-display text-xl font-extrabold">
                <CmsText id="Footer.c9db85fc4db0">ScholarsCafe</CmsText>
              </span>
            </Link>
            <p className="mt-4 max-w-sm text-sm leading-relaxed text-white/55">
              <CmsText id="Footer.ecef8dd200d7">
                Your gateway to global education. Scholars Cafe is Bangladesh's dedicated study
                abroad consultancy, helping ambitious students reach their dream universities
                worldwide.
              </CmsText>
            </p>
            <div className="mt-5 flex flex-wrap gap-2">
              <FootIcon href="https://www.facebook.com/profile.php?id=61571394902795">
                <Facebook className="h-4 w-4" />
              </FootIcon>
              <FootIcon href="https://www.instagram.com/scholars_cafe_/">
                <Instagram className="h-4 w-4" />
              </FootIcon>
              <FootIcon href="https://www.linkedin.com/company/scholars-cafe09/">
                <Linkedin className="h-4 w-4" />
              </FootIcon>
              <FootIcon href={WA_LINK}>
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.198-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487 2.981 1.287 2.981.858 3.518.804.537-.054 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413" />
                </svg>
              </FootIcon>
              <a
                href={`mailto:${EMAIL}`}
                className="flex h-9 items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-3 text-xs font-medium text-white/70 transition-colors hover:bg-white/10"
              >
      ...
      <div className="border-t border-white/7">
        <div className="mx-auto flex max-w-[1280px] flex-col items-center justify-between gap-3 px-5 py-7 text-[13px] text-white/40 md:flex-row md:px-8">
          <div className="flex items-center gap-1.5 text-[13px] text-white/40">
            <span>
              <CmsText id="Footer.0a79b0ca699d">© </CmsText>
              {new Date().getFullYear()}
              <CmsText id="Footer.284e1c10b562"> Scholars Cafe. All rights reserved.</CmsText>
            </span>
          </div>
          <div className="flex gap-5">
            <Link to="/privacy" className="transition-colors hover:text-white">
              <CmsText id="Footer.54a57c3147c4">Privacy</CmsText>
            </Link>
            <Link to="/terms" className="transition-colors hover:text-white">
              <CmsText id="Footer.ede548996483">Terms</CmsText>
            </Link>
            <Link to="/cookies" className="transition-colors hover:text-white">
              <CmsText id="Footer.141395eb3556">Cookies</CmsText>
            </Link>
          </div>
        </div>
      </div>
    ...
    <div>
      ...
      <ul className="mt-4 space-y-2.5 text-sm text-white/55">
        {links.map((l) => (
          <li key={l.label}>
            {l.route ? (
              <Link to={l.route} className="transition-colors hover:text-sky-light">
                {l.label}
              </Link>
            ) : (
              <a
