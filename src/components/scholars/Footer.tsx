...
export function Footer() {
 const cmsLabel = useCmsLookup();
  ...
  return (
    <footer className="border-t border-sky/15 bg-sky-soft text-navy">
      <div className="mx-auto max-w-[1280px] px-5 py-16 md:px-8">
        <div className="grid grid-cols-2 gap-10 md:grid-cols-4">
          <div className="col-span-2">
            <Link to="/" className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white shadow-card" aria-label="Scholars Cafe home">
              <span className="brand-mark" aria-hidden="true" />
            </Link>
            <p className="mt-4 max-w-sm text-sm leading-relaxed text-navy/65"><CmsText id="Footer.ecef8dd200d7">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <a href={`mailto:${EMAIL}`} className="flex h-9 items-center gap-2 rounded-xl border border-sky/20 bg-white px-3 text-xs font-medium text-navy/70 transition-all hover:-translate-y-0.5 hover:border-sky">
        ...
        <div className="mt-12 grid grid-cols-2 gap-10 border-t border-sky/15 pt-10 md:grid-cols-4">
          <Col
      ...
      <div className="border-t border-sky/15">
        <div className="mx-auto flex max-w-[1280px] flex-col items-center justify-between gap-3 px-5 py-7 text-[13px] text-navy/50 md:flex-row md:px-8">
          <div className="flex items-center gap-1.5 text-[13px] text-navy/50">
            ...
            <Heart className="inline h-3.5 w-3.5 fill-sky text-sky" aria-hidden />
            <span><CmsText id="Footer.66c9cc3a6577">in Bangladesh.</CmsText></span>
          </div>
          <div className="flex gap-5">
            <Link to="/privacy" className="transition-colors hover:text-sky"><CmsText id="Footer.54a57c3147c4">Privacy</CmsText></Link>
            <Link to="/terms" className="transition-colors hover:text-sky"><CmsText id="Footer.ede548996483">Terms</CmsText></Link>
            <Link to="/cookies" className="transition-colors hover:text-sky"><CmsText id="Footer.141395eb3556">Cookies</CmsText></Link>
          </div>
        </div>
      </div>
    </footer>
...
function Col({ title, links }: { title: string; links: LinkRef[] }) {
  ...
  return (
    <div>
      <div className="font-display text-sm font-bold uppercase tracking-wider text-navy">{title}</div>
      <ul className="mt-4 space-y-2.5 text-sm text-navy/60">
        {links.map((l) => (
          <li key={l.label}>
            {l.route ? (
              <Link to={l.route} className="transition-colors hover:text-sky">{l.label}</Link>
            ) : (
              <a
                href={l.href}
...
function FootIcon({ href, children }: { href: string; children: React.ReactNode }) {
  ...
  return (
    <a
      ...
      className="flex h-9 w-9 items-center justify-center rounded-xl border border-sky/20 bg-white text-navy/70 transition-all hover:-translate-y-0.5 hover:border-sky hover:bg-sky hover:text-white"
    >
      {children}
    </a>
  );
