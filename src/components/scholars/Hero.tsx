...
export function Hero() {
 const cmsLabel = useCmsLookup();
  ...
  return (
    <section
      id="top"
      className="relative isolate overflow-hidden bg-sky-soft pt-24 md:pt-28"
    >
      ...
      <div className="absolute inset-0 -z-20">
        <picture>
          ...
          <img
            ...
            width={1920}
            height={1280}
            className="h-full w-full object-cover object-top opacity-20 md:object-center md:opacity-25"
          />
        </picture>
      </div>
      ...
      <div className="relative mx-auto max-w-[1320px] px-5 md:px-10">
        ...
        <div className="mt-8 grid grid-cols-1 gap-10 lg:mt-10 lg:grid-cols-[1.5fr_1fr] lg:gap-16">
          <div>
            <motion.p
              ...
              className="mb-4 font-display italic text-lg text-sky md:text-xl"
            ><CmsText id="Hero.f3f277c44b6e">
              Thousands Apply. Few Stand Out.
            </CmsText></motion.p>
            <h1 className="font-display text-[clamp(2.35rem,14vw,7rem)] font-extrabold leading-[0.95] tracking-[-0.035em] text-navy md:text-[clamp(2.75rem,8vw,7rem)] md:leading-[0.92]">
              {headline.map((word, i) => (
                <motion.span
                  key={word}
              ...
              <motion.span
                ...
                className="font-display block italic font-light text-sky"
            ...
            <motion.p
              ...
              className="mt-6 max-w-xl text-[16px] leading-[1.7] text-navy/70 md:mt-10 md:text-[17px]"
            ...
            >
              ...
              <a
                href="#programs"
                className="group inline-flex w-full items-center justify-center gap-2 px-2 py-3 text-[15px] font-semibold text-navy sm:w-auto sm:py-4"
              >
                <span className="border-b border-navy/30 pb-0.5 transition-colors group-hover:border-sky group-hover:text-sky"><CmsText id="Hero.f4685c1f868b">
                  Explore programs
                </CmsText></span>
        ...
        >
          <div className="mb-5 flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.22em] text-navy/55">
            <span className="h-px w-10 bg-sky" /><CmsText id="Hero.a73b267ead09">
...
function AcceptanceRailItem({ name, logo, onClick }: AcceptanceRailItemProps) {
  ...
  return (
    <button type="button" onClick={onClick} className="inline-flex shrink-0 items-center gap-3 whitespace-nowrap px-1 py-2 text-navy/60 transition-all hover:-translate-y-0.5 hover:text-sky">
      <img
        src={logo}
        alt={`${name} logo`}
        loading="lazy"
        decoding="async"
        className="h-10 w-10 shrink-0 object-contain opacity-70 md:h-11 md:w-11"
      />
