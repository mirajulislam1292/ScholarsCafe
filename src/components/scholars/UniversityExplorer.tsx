...
import { UNIVERSITIES as CMS_DEFAULT_UNIVERSITIES, WA_LINK as CMS_DEFAULT_WA_LINK, type University } from "@/lib/scholars-data";
...
export function UniversityExplorer() {
 const cmsLabel = useCmsLookup();
  ...
  }, [country, scholarship, type, tuition, search, UNIVERSITIES]);
    ...
    <section id="universities" className="bg-white py-24 md:py-32">
      <div className="mx-auto max-w-[1280px] px-5 md:px-8">
        <Reveal className="mx-auto max-w-2xl text-center">
          <span className="text-xs font-bold uppercase tracking-[0.15em] text-sky"><CmsText id="UniversityExplorer.0d7a9a83ef6d">University Explorer</CmsText></span>
          <h2 className="mt-4 font-display text-[clamp(2rem,3.5vw,3rem)] font-extrabold text-navy"><CmsText id="UniversityExplorer.3bbe267caa26">
            Find Your Perfect Fit
          </CmsText></h2>
          <p className="mt-5 text-[17px] leading-relaxed text-slate-600"><CmsText id="UniversityExplorer.87c9a85ac63f">
        ...
        <Reveal>
          <div className="mt-12 rounded-3xl border border-border bg-white p-6 shadow-card md:p-8">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <Select label={cmsLabel("UniversityExplorer.label.100163c6400d","Scholarship")} value={scholarship} onChange={setScholarship} options={SCHOLARSHIPS} />
              ...
              <div className="relative">
                <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-slate-500"><CmsText id="UniversityExplorer.49c266baaaa7">Search</CmsText></label>
            ...
            {activeTags.length > 0 && (
              <div className="mt-5 flex flex-wrap items-center gap-2">
                ...
                <button onClick={clear} className="text-xs font-medium text-slate-500 underline-offset-2 hover:text-navy hover:underline"><CmsText id="UniversityExplorer.29a390f9237e">
                  Clear all
                </CmsText></button>
              </div>
            )}
            <div className="mt-4 text-sm text-slate-500">{filtered.length}<CmsText id="UniversityExplorer.ebca61909226"> universities found</CmsText></div>
          </div>
        </Reveal>
          ...
          {filtered.slice(0, visible).map((u) => (
            ...
            >
              ...
              <div className="mt-3 space-y-1 text-[12px] text-slate-600">
                ...
                <div><CmsText id="UniversityExplorer.172b6f9f21d3">Acceptance: </CmsText>{u.acceptance}</div>
              </div>
            </button>
          ))}
        </div>
        ...
        {filtered.length > visible && (
          <div className="mt-10 text-center">
            ...
            ><CmsText id="UniversityExplorer.8f9849c015cd">
      ...
      {selected && (
        ...
        >
          ...
          >
            <button onClick={() => setSelected(null)} className="absolute right-5 top-5 text-slate-400 hover:text-navy" aria-label={cmsLabel("UniversityExplorer.label.7d9eb7acb13e","Close")}>
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="mt-1 text-sm text-slate-500">{selected.country}<CmsText id="UniversityExplorer.a137f17a19a0"> · </CmsText>{selected.type}</div>
            ...
            <p className="mt-6 text-[14px] leading-relaxed text-slate-600">
              {selected.name}<CmsText id="UniversityExplorer.192eae1272bb"> is on our recommended list for our Full Scholarship and General Admission tracks.
